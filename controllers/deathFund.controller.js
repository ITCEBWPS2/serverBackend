...
export const createDeathFund = async (req, res) => {
  try {
    ...
    if (!updatedMember) {
      await Logger.warn(
        "com.ceb.deathfundctrl.createDeathFund",
        "Member not found when creating death fund",
        req.user?._id || null,
        {
          epf,
          ip: req.ip,
    ...
    await Logger.info(
      "com.ceb.deathfundctrl.createDeathFund",
      "Death fund created and linked successfully",
      req.user?._id || null,
      {
        epf,
        ip: req.ip,
    ...
    await Logger.error(
      "com.ceb.deathfundctrl.createDeathFund",
      "Error creating death fund - " + error.message,
      req.user?._id || null,
      {
        ip: req.ip,
        userAgent: req.get("User-Agent"),
...
export const viewAllDeathFunds = async (req, res) => {
  try {
    ...
    await Logger.info(
      "com.ceb.deathfundctrl.viewAllDeathFunds",
      "Fetched all death funds",
      req.user?._id || null,
      {
        ip: req.ip,
        userAgent: req.get("User-Agent"),
    ...
    await Logger.error(
      "com.ceb.deathfundctrl.viewAllDeathFunds",
      "Error fetching all death funds - " + error.message,
      req.user?._id || null,
      {
        ip: req.ip,
        userAgent: req.get("User-Agent"),
...
export const viewSingleDeathFund = async (req, res) => {
  try {
    ...
    if (!benefit) {
      await Logger.warn(
        "com.ceb.deathfundctrl.viewSingleDeathFund",
        "Death fund not found",
        req.user?._id || null,
        {
          id: req.params.id,
          ip: req.ip,
    ...
    await Logger.error(
      "com.ceb.deathfundctrl.viewSingleDeathFund",
      "Error fetching single death fund - " + error.message,
      req.user?._id || null,
      {
        ip: req.ip,
        userAgent: req.get("User-Agent"),
...
export const updateDeathFund = async (req, res) => {
  try {
    ...
    if (!updatedBenefit) {
      await Logger.warn(
        "com.ceb.deathfundctrl.updateDeathFund",
        "Death fund not found for update",
        req.user?._id || null,
        {
          id: req.params.id,
          ip: req.ip,
    ...
    await Logger.error(
      "com.ceb.deathfundctrl.updateDeathFund",
      "Error updating death fund - " + error.message,
      req.user?._id || null,
      {
        ip: req.ip,
        userAgent: req.get("User-Agent"),
...
export const deleteDeathFund = async (req, res) => {
  try {
    ...
    if (!benefit) {
      await Logger.warn(
        "com.ceb.deathfundctrl.deleteDeathFund",
        "Death fund not found for deletion",
        req.user?._id || null,
        {
          id: req.params.id,
          ip: req.ip,
    ...
    await Logger.error(
      "com.ceb.deathfundctrl.deleteDeathFund",
      "Error deleting death fund - " + error.message,
      req.user?._id || null,
      {
        ip: req.ip,
        userAgent: req.get("User-Agent"),
...
export const getBenefitsByUserId = async (req, res) => {
  try {
    ...
    if (!member) {
      await Logger.warn(
        "com.ceb.deathfundctrl.getBenefitsByUserId",
    ...
    await Logger.info(
      "com.ceb.deathfundctrl.getBenefitsByUserId",
      "Retrieved death fund benefits by user",
      req.user?._id || null,
      {
        ip: req.ip,
        userAgent: req.get("User-Agent"),
    ...
    await Logger.error(
      "com.ceb.deathfundctrl.getBenefitsByUserId",
      "Error retrieving benefits - " + error.message,
      req.user?._id || null,
      {
        ip: req.ip,
        userAgent: req.get("User-Agent"),
